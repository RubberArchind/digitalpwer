...
if ($result !== false && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($dayleft > 0) {
            ...
            $conn->query($sql5);
            $sql3 = sprintf("INSERT INTO logs(user_id, type, amount, ref) VALUES ('%s', '%s', %f, '%s')", $row['user_id'], 'CASHBACK', $row['amount'] * 0.1, $row['id']);
            $conn->query($sql3);
            $sql4 = sprintf("INSERT INTO transaction(id, user_id, target_id, method, type, amount) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')", "".rand(), $row['user_id'], $row['user_id'], 'Internal', 'CASHBACK', $row['amount'] * 0.1);
            $conn->query($sql4);
            $sql2 = sprintf("UPDATE user SET balance_cashback = balance_cashback + %f WHERE user_id='%s'", $row['amount']*0.1, $row['user_id']);
            $conn->query($sql2);
