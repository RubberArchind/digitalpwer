...
if ($result !== false && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        while ($dayleft != 0) {
            ...
            $sql3 = sprintf("INSERT INTO logs(user_id, type, amount, time, ref) VALUES ('%s', '%s', %f, '%s', '%s')", $row['user_id'], 'CASHBACK', $row['amount'] * 0.1, $c->format("Y-m-d H:i:s"), $row['id']);
            $conn->query($sql3);
            $sql4 = sprintf("INSERT INTO transaction(id, user_id, target_id, method, type, amount, time) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s')", "".rand(), $row['user_id'], $row['user_id'], 'Internal', 'CASHBACK', $row['amount'] * 0.1, $c->format("Y-m-d H:i:s"));
            $conn->query($sql4);
